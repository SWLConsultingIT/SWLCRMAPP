  ...
  return (
    <div>
      <div className="flex items-baseline gap-2 mb-1">
        ...
        <span style={{ fontSize: 12, color: C.textMuted }}>{t("cons.ov.replyEvents")}</span>
        <span style={{ fontSize: 12, color: C.textDim }}>·</span>
        <span className="font-semibold tabular-nums" style={{ fontSize: S.minor, color: C.textBody }}>{q.leads}</span>
        <span style={{ fontSize: 12, color: C.textMuted }}>{t("cons.ov.leads")}</span>
      </div>
    ...
    <div>
      <div className="flex items-baseline gap-5 mb-3 flex-wrap">
        <span className="inline-flex items-baseline gap-1.5">
          ...
          <span style={{ fontSize: 11.5, color: C.textMuted }}>{t("cons.ov.sent")}</span>
        ...
        <span className="inline-flex items-baseline gap-1.5">
          ...
          <span style={{ fontSize: 11.5, color: C.textMuted }}>{t("cons.ov.replies")}</span>
          <span className="tabular-nums font-semibold" style={{ fontSize: 13, color: C.textPrimary }}>{totals.replies}</span>
        </span>
        <span style={{ fontSize: 11, color: C.textDim }}>{t("cons.ov.dashedPrior")}</span>
      </div>
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full block" style={{ height: "auto", aspectRatio: `${W} / ${H}`, maxHeight: 210 }} role="img" aria-label={t("cons.ov.trendAria")}>
...
function Timing() {
  ...
  const { t } = useLocale();
  const tm = D.timing;
  const max = Math.max(...tm.grid.flat(), 1);
  const totals = tm.grid.map(r => r.reduce((a, b) => a + b, 0));
  const bestDay = totals.indexOf(Math.max(...totals));
  const grand = tm.grid.flat().reduce((a, b) => a + b, 0);
  return (
    <div>
      <div className="flex gap-1.5 mb-1.5" style={{ paddingLeft: 42 }}>
        {tm.blocks.map(b => <span key={b} className="flex-1 text-center tabular-nums" style={{ fontSize: 11, color: C.textMuted }}>{b}</span>)}
        <span style={{ width: 34 }} />
      </div>
      {tm.grid.map((row, i) => (
        <div key={tm.days[i]} className="flex items-center gap-1.5 mb-1.5">
          <span className="shrink-0" style={{ width: 38, fontSize: 11.5, color: i === bestDay ? C.textPrimary : C.textMuted }}>{tm.days[i]}</span>
          {row.map((v, j) => (
            <div key={j} className="flex-1 rounded-md flex items-center justify-center" title={`${tm.days[i]} ${tm.blocks[j]} · ${v} replies`}
  ...
  return (
    <div>
      <div className="flex items-baseline gap-3 mb-4 flex-wrap">
        ...
        <span style={{ fontSize: 13, color: C.textBody }}>{t("cons.ov.leadsInWorkspace")}</span>
        <div className="flex-1" />
    ...
    <>
      ...
      <section className="pt-7">
        <div className="flex items-baseline gap-4 flex-wrap pb-1">
          <h2 className="font-semibold tracking-tight" style={{ fontSize: 17, color: C.textPrimary }}>{t("cons.ov.q1")}</h2>
          <span style={{ fontSize: 12.5, color: C.textMuted }}>{t("cons.ov.in", { label })}</span>
          <div className="flex-1" />
          <Drill label={t("cons.tab.campaigns")} />
        </div>
        <Flow />
      ...
      <Band question={t("cons.ov.q2")} aside={<Drill label={t("cons.tab.channels")} />}>
        <Channels />
      </Band>
      ...
      <Band question={t("cons.ov.q3")} aside={<Drill label={t("cons.tab.sellers")} />}>
        <div className="grid grid-cols-1 lg:grid-cols-2" style={{ gap: 52 }}>
          <div>
            <div className="font-semibold uppercase tracking-wider mb-4" style={{ fontSize: S.label, color: C.textMuted }}>{t("cons.tab.campaigns")} <span className="font-normal normal-case tracking-normal" style={{ color: C.textDim }}>{t("cons.ov.sortedBy", { by: D.rankedBy.toLowerCase() })}</span></div>
            <Ranked rows={D.campaigns} note={D.campaignsNote} />
          </div>
          <div className="lg:pl-10" style={{ borderLeft: `1px solid ${C.border}` }}>
            <div className="font-semibold uppercase tracking-wider mb-4" style={{ fontSize: S.label, color: C.textMuted }}>{t("cons.tab.sellers")} <span className="font-normal normal-case tracking-normal" style={{ color: C.textDim }}>{t("cons.ov.sortedBy", { by: D.rankedBy.toLowerCase() })}</span></div>
            <Ranked rows={D.sellers} note={D.sellersNote} />
          </div>
        </div>
      </Band>

      <Band question={t("cons.ov.q4")}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12" style={{ gap: 48 }}>
          <div className="lg:col-span-5">
            <div className="font-semibold uppercase tracking-wider mb-5" style={{ fontSize: S.label, color: C.textMuted }}>
              {t("cons.ov.replyQuality")} <Def text={t("cons.ov.replyQualityDef")} />
            </div>
            <ReplyQuality />
          </div>
          <div className="lg:col-span-7 lg:pl-11" style={{ borderLeft: `1px solid ${C.border}` }}>
            <div className="font-semibold uppercase tracking-wider mb-5" style={{ fontSize: S.label, color: C.textMuted }}>{t("cons.ov.activity")}</div>
            <Trend />
          </div>
        </div>

        <div className="mt-14 pt-9" style={{ borderTop: `1px solid ${C.border}` }}>
          <div className="font-semibold uppercase tracking-wider mb-5" style={{ fontSize: S.label, color: C.textMuted }}>{t("cons.ov.whenReply")}</div>
          <Timing />
        </div>
      </Band>
    </>
