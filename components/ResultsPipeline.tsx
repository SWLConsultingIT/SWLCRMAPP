              ...
              >
                ...
                <div className="relative" style={{ background: `linear-gradient(135deg, ${N.ink2}, ${N.ink})` }}>
                  ...
                  <div className="px-3.5 py-2.5 flex items-center gap-2">
                    ...
                    <span className="text-[12px] font-bold flex-1 truncate" style={{ color: "#fff", fontFamily: BRAND_FONT }}>{stageLabel(col, t)}</span>
                ...
                <div className="p-2 space-y-2 flex-1" style={{ minHeight: 160 }}>
                  {cards.length === 0 ? (
                    <div className="rounded-xl border border-dashed text-center text-[11px] py-10 select-none" style={{ borderColor: C.border, color: C.textDim }}>
                      {isOdoo ? t("oppPipe.dropToOdoo") : t("oppPipe.dropLead")}
                    </div>
                  ) : cards.map(lead => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between mt-2.5">
                          {lead.transferred ? (
                            <span className="inline-flex items-center gap-1 text-[9.5px] font-bold px-1.5 py-0.5 rounded" style={{ backgroundColor: C.greenLight, color: C.green }}><Trophy size={8} /> {t("oppPipe.inOdoo")}</span>
                          ) : lead.days_to_convert != null ? (
                            <span className="text-[9.5px] font-semibold tabular-nums px-1.5 py-0.5 rounded" style={{ backgroundColor: `color-mix(in srgb, ${gold} 13%, transparent)`, color: C.goldDim }}>{lead.days_to_convert}d {t("oppPipe.toReply")}</span>
                          ) : <span />}
                          <Link href={`/opportunities/${lead.id}`} className="inline-flex items-center gap-0.5 text-[10.5px] font-semibold opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: gold }}>
                            {t("oppPipe.details")} <ChevronRight size={11} />
                          </Link>
                        </div>
                      </div>
                    );
