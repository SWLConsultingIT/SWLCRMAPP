...
// Realtime) — no new surface.
//
// BOUNDARY: notification infrastructure is NOT part of this feature. This
// module is a CONSUMER of it (the `notifications` table, lib/web-push, SMTP)
// and owns only the activity-side rules: which rows are due, the claim, and
// what the reminder says. Framework-free on purpose — it returns a status +
...
export type ReminderRunResult = { status: number; body: Record<string, unknown> };
...
export async function runActivityReminders(): Promise<ReminderRunResult> {
  ...
  if (rows.length === 0) return { status: 200, body: { ok: true, fired: 0 } };
  ...
  const { error: claimErr } = await svc
    ...
    .in("id", ids)
    .is("reminder_sent_at", null);
  if (claimErr) return { status: 500, body: { error: claimErr.message } };
  ...
  if (notifErr) return { status: 500, body: { error: notifErr.message, fired: 0, claimed: ids.length } };
  ...
  return { status: 200, body: { ok: true, fired: notifications.length, emailed } };
}
